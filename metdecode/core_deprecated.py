...
from metdecode.bounds import ConstraintSet
...
class MetDecode:
    ...
    def fit(
            ...
            M_cfdna: np.ndarray,
            D_cfdna: np.ndarray,
            bounds: Optional[ConstraintSet] = None,
            n_unknown_tissues: int = 0,
            max_n_iter: int = 2000,
            patience: int = 100
    ) -> 'MetDecode':
        ...
        optimizer.add(list(bias_correction.parameters()), 1e-3, weight_decay=0.00001)
        ...
        for it in tqdm.tqdm(range(max_n_iter), desc='Correcting atlas'):
            ...
            loss = torch.sum(weights_cfdna * torch.square(R_reconstructed - R_cfdna))

            # Methylation ratios should stay close to the original atlas
            # g2 = torch.max(torch.mean(torch.abs(R_atlas - gamma_corrected), dim=1))
            g2 = torch.max(torch.square(torch.mean(torch.abs(R_atlas - gamma_corrected[:n_known_tissues, :]), dim=1) - self.budget))
